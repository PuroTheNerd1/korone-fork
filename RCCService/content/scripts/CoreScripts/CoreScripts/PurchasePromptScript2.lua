	...
	if IsNativePurchasing then
		...
		if not ThirdPartyProductName then
			if isTenFootInterface then
				-- don't direct them to korone.zip on consoles.
				...
				descriptionText = "This item cost more ROBUX than you can purchase. Please visit www.korone.zip to purchase more ROBUX."
			end
...
local function onPurchaseFailed(failType)
	...
	elseif failType == PURCHASE_FAILED.NOT_ENOUGH_TIX then
		failedText = "This item cost more tickets than you currently have. Try trading currency on www.korone.zip to get more tickets."
		setPreviewImage(PurchaseData.ProductInfo, PurchaseData.AssetId)
	...
	elseif failType == PURCHASE_FAILED.LIMITED then
		failedText = "This limited item has no more copies. Try buying from another user on www.korone.zip. Your account has not been charged."
		setPreviewImage(PurchaseData.ProductInfo, PurchaseData.AssetId)
	...
	elseif failType == PURCHASE_FAILED.PROMPT_PURCHASE_ON_GUEST then
		failedText = "You need to create a ROBLOX account to buy items, visit www.korone.zip for more info."
