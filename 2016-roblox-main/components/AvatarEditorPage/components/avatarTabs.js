...
const avatarTabs = props => {
    const s = useStyles();
    const {options} = props;
    const {openSubmenu, setOpenSubmenu} = AvatarPageStore.useContainer();
    const [tabMouseHover, setTabMouseHover] = useState(false);
    ...
    useEffect(() => {
        if (tabMouseHover && !openSubmenu && currentHovered) {
            console.log("HOVER!");
            setOpenSubmenu(currentHovered);
        }
    }, [tabMouseHover])
        ...
        <div className={`${s.buttonCol} col-12`}>
            {
                options.map(option => {
                    ...
                    return <div key={option.name} className={s.vTab} onClick={() => {
                        ...
                        setTabMouseHover(true);
                    }} onMouseLeave={() => {
                        setCurrentHovered(null);
                        setTabMouseHover(false);
                    }}>
