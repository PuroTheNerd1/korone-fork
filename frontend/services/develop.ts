...
export const setAssetPrice = async ({ assetId, priceInRobux, priceInTickets }: {assetId: number, priceInRobux?: number, priceInTickets?: number}) => {
    let obj = {
        priceInRobux,
        priceInTickets
    };
...
export const getAssetRestrictions = async (assetIds) => {
    let req = await request("GET", getFullUrl('itemconfiguration',
        `/v1/assets/restrictions?assetIds=${encodeURIComponent(assetIds.join(","))}`)
    );
