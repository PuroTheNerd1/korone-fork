...
const DescriptionEntry = (props) => {
  const deletedMessage = `This item was deleted. For more details, contact customer support, and reference transaction ID: #${
    props.id || "-"
  }`;
  let verb = "";
  let link = "";
  let noun: string | JSX.Element = "";
    ...
    <div>
      <Link href={`/users/${props.id}/profile`}>
        <a>
          ...
          <div className={s.sellerName}>{props.name}</div>
        </a>
      </Link>
    </div>
  );
...
const MyTransactionsTable = (props) => {
  ...
  const [type, setType] = useState(
    props.creatorType === "Group" ? "sale" : "purchase"
  );
  ...
  return (
    <div className="row">
      ...
      <div className="col-12">
        <Table
          keys={["Date", "Member", "Description", "Amount"]}
          entries={
            entries &&
            entries.data &&
            entries.data.map((v) => {
      ...
      <div className="col-12">
        {entries && entries.nextPageCursor && (
          <p
            className={s.more}
            onClick={() => {
