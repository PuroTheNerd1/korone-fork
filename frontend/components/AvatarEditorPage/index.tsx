...
function AvatarEditor() {
  ...
  async function AssetTypeClick(item, e) {
    if (debounce.current || listItemMetadata.current.assetType === item.typeId)
      return;
  ...
  async function RecentClick(item, e) {
    // DO NOT PUT === HERE!!
    if (debounce.current || listItemMetadata.current.recentType == item.typeId)
      return;
    debounce.current = true;
    page.setSelectedList({
      tab: item.tabId,
  ...
  useEffect(() => {
    (async () => {
      if (store.isRendering || page.thumbnailType !== 1 || !store.avThumb3D) {
        ...
        page.thumbnailType === 1 &&
        !thumbnail3D.current.isLoadingThumbnail
      ) {
        await thumbnail3D.current.LoadThumbnail(
          store.avThumb3D,
  ...
  return (
    <div>
      <div
        className={`${s.avatarHeader} flex justify-content-between align-items-center`}
      >
      ...
      <div className={`flex ${s.content}`}>
        <div className={s.idekbuh}>
          <div className={`section-content ${s.contentContainer}`}>
            <div className={s.avatarThumbContainer}>
              ...
              ) : store.avThumb3D && is3DReady ? null : (
                <span
                  className="spinner"
              ...
              <div className={s.avatarRigTypeSelector}>
                <RadioPill
                  options={["R6", "R15"]}
                  selected={store?.bodyRigType}
                  setSelected={store?.setModifiedRigType}
                />
              </div>
              <div className={s.thumbnail3DButtonContainer}>
                <ActionButton
                  ...
                  onClick={() =>
                    page.LoadNewThumbnailType(page.thumbnailType === 1 ? 0 : 1)
                  }
                />
              </div>
            </div>
            <div
              className={`${s.scalingContainer} ${s.scalingContainerDesktop}`}
            >
          ...
          <div
            className={`flex justify-content-between ${s.redrawContainer} ${s.firstRedraw}`}
          >
            ...
            options={[
              {
                ...
                name: (
                  <span>
                    Recent <span className={`icon-down ${s.iconDown}`} />
                  </span>
                ),
                element: (
                  <AvatarTabSubmenu
                    ...
                    onButtonClick={async (item, e) =>
                      await RecentClick(item, e)
                    }
                  />
                ),
              },
              {
                ...
                name: (
                  <span>
                    Clothing <span className={`icon-down ${s.iconDown}`} />
                  </span>
                ),
                element: (
                  <AvatarTabSubmenu
                    data={[
                      ...
                      },
                      {
                        label: "Gear",
                        items: [
                          { name: "Gear", typeId: 19, tabId: "clothing" },
                        ],
                      },
                    ]}
                    onButtonClick={async (item, e) =>
                      await AssetTypeClick(item, e)
                    }
              ...
              },
              {
                ...
                name: (
                  <span>
                    Body <span className={`icon-down ${s.iconDown}`} />
                  </span>
                ),
                element: (
                  <AvatarTabSubmenu
              ...
              },
              {
                ...
                name: (
                  <span>
                    Animations <span className={`icon-down ${s.iconDown}`} />
                  </span>
                ),
                element: (
                  <AvatarTabSubmenu
                ...
                return (
                  <div>
                    <div style={{ display: "flex" }}>
                      <span style={{ paddingTop: 9, paddingBottom: 4 }}>
                        {CapitalizeVariable(page.selectedList.tab)}
                        {!IsNullOrEmpty(page?.selectedList?.subTab) &&
                          ` > ${CapitalizeVariable(
                            page?.selectedList?.subTab
                          )}`}
                      </span>
                    </div>
                    <AvatarCardList />
        ...
        <div
          className={`section-content ${s.scalingContainerMobile}`}
          style={{ padding: 0 }}
        >
