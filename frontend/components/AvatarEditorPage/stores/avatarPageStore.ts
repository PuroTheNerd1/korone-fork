...
const AvatarPageStore = createContainer(() => {
  ...
  const [listItemMetadata, setListItemMetadata] = useState<ItemListMetadata>(
    {}
  );
  ...
  async function LoadRecentItemsToList(type) {
    ...
    let thumbnails = await multiGetAssetThumbnails({
      assetIds: recent.map((item) => item.id),
    });
    let itemRestrictions = await getItemRestrictions(
  ...
  async function LoadAssetTypeToList(type: number, loadMore?: boolean) {
    setLoadingAvatar(true);
    ...
    let thumbnails = await multiGetAssetThumbnails({
      assetIds: invList.Items.map((item) => item.Item.AssetId),
    });
    let itemRestrictions = await getItemRestrictions(
      invList.Items.map((item) => item.Item.AssetId)
    );
    let newItems = invList.Items.map((item) => {
      let thumb =
        thumbnails?.find((v) => v.targetId === item.Item.AssetId) || null;
  ...
  async function LoadOutfits() {
    ...
    let outfitThumbnails = await multiGetOutfitThumbnails({
      userOutfitIds: outfits.data.map((v) => v.id),
      size: "100x100",
    });
    setOutfits(
      outfits.data.map((outfit) => {
        let outfitThumb =
          outfitThumbnails?.find((v) => v.targetId === outfit.id) || null;
        return {
          name: outfit.name,
          outfitId: outfit.id,
