...
const AvatarInfoStore = createContainer(() => {
  ...
  const [avRules, setAvRules] = useState<AvatarRules>(null);
  ...
  useEffect(() => {
    (async () => {
      if (debo.current || !isRendering || avThumb != null || avThumb3D != null)
        return;
      debo.current = true;
      ...
      while (avThumb == null && attempts <= 10) {
        let thumbnail = await multiGetUserThumbnails({
          userIds: [auth.userId],
      ...
      if (attempts > 10 && avThumb == null)
        feedback.addFeedback(
          "Could not get new avatar render. Please try again later.",
          FeedbackType.ERROR
        );
      console.log(
        `Got avatar render in ${stopwatch.ElapsedMilliseconds()}ms, in ${attempts} attempts.`
      );
      ...
      while (avThumb3D == null && attempts <= 10) {
        let thumbnail = await multiGetUserThumbnails3D({
          userIds: [auth.userId],
        }).then((result) => result[0]);
        if (
          thumbnail.state === "Completed" &&
      ...
      console.log(
        `Got 3D avatar render in ${stopwatch.ElapsedMilliseconds()}ms, in ${attempts} attempts.`
      );
  ...
  useEffect(() => {
    ...
    if (!IsNegative(newAsset.assetId)) {
      if (AssetTypeCategory.LimitToOne.includes(newAsset.assetType)) {
        for (const assetType of AssetTypeCategory.LimitToOne) {
          let onlyOneAllowed = wearingAssets.filter(
            /** @param {WearingAsset} asset */ (asset) => {
              return (
    ...
    setWearingAssets((prev) => {
      ...
      (async () => {
        await AvatarService.setWearingAssets({
          assetIds: updated.map((d) => d.assetId),
        });
        await GetUpdatedAvatar();
      })();
...
export const AssetTypeCategory = {
  ...
  LimitToOne: [
    50, 51, 52, 53, 54, 55, 17, 27, 28, 29, 30, 31, 18, 19, 12, 11, 2,
  ],
  Unlimited: [24],
  All: [
    50, 51, 52, 53, 54, 55, 17, 27, 28, 29, 30, 31, 32, 18, 19, 12, 11, 2, 24,
    61, 8, 41, 42, 43, 44, 45, 46, 47,
  ],
};
