...
function OutfitCard({
  outfit,
  setDeleteOutfitModal,
  setRenameOutfitModal,
  setUpdateOutfitModal,
}) {
      ...
      onClick: () => {
        if (!outfit.thumbnail) {
          feedback.addFeedback(
            "Failed to download outfit thumbnail",
            FeedbackType.ERROR
          );
          return;
        }
  ...
  return (
    <div className={`${s.avatarCardWrapper}`}>
      <div className={`${s.avatarCardContainer}`}>
        <div
          className={s.avatarCardImage}
          onClick={async () => {
        ...
        >
          <img
            src={ThumbnailFromState(outfit.thumbnail, outfit.thumbnailState)}
            alt={outfit.name}
          />
        </div>
        <div className={s.avatarCardItemLinkContainer}>
          <Link href="#" className={s.avatarCardItemLink}>
        ...
        <div
          ...
          style={openSettings ? { display: "flex" } : null}
        >
          ...
          <span style={{ marginTop: 3, fontSize: 10 }}>
            {dayjs(outfit.createdAt).format("MM/D/YYYY")}
          </span>
        </div>
      </div>
    </div>
...
function OutfitsTab() {
  ...
  return (
    <div className={s.outfitsWrapper}>
      {deleteOutfitModal ? (
        <DeleteOutfitModal
          outfitId={deleteOutfitModal}
          openModal={setDeleteOutfitModal}
        />
      ) : null}
      {updateOutfitModal ? (
        <UpdateOutfitModal
          outfitId={updateOutfitModal}
          openModal={setUpdateOutfitModal}
        />
      ) : null}
      {renameOutfitModal ? (
        <RenameOutfitModal
          outfitId={renameOutfitModal}
          openModal={setRenameOutfitModal}
        />
      ) : null}
      {createOutfitModal ? (
        <CreateOutfitModal
          openModal={setCreateOutfitModal}
        />
      ) : null}
      ...
      <div className={s.outfitsContainer}>
        ...
        {!loadingOutfits && page.outfits.length === 0 && (
          <span className={`section-content-off w-100`}>
        ...
        {loadingOutfits && (
          <span
            ...
            style={{ height: "36px", backgroundSize: "auto 36px" }}
          />
        )}
      </div>
    </div>
