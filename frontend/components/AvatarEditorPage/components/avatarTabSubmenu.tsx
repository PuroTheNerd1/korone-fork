 ...
 */
interface SubmenuNestedData {
    label: string;
    items: SubmenuData[];
};

export enum SUBMENU_MODE {
    DEFAULT = 0,
    NESTED = 1
};
 ...
 * @constructor
 */
const AvatarSubmenu = ({data, onButtonClick, mode = SUBMENU_MODE.DEFAULT }: {data: SubmenuData[] | SubmenuNestedData[], onButtonClick: (...any) => void, mode?: SUBMENU_MODE}) => {
    const s = useStyles();
    ...
    return <>
        ...
        }
        {
            ...
            data.map(item => {
                let nest: SubmenuNestedData = item;
