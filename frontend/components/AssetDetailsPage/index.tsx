...
function AssetDetailsPage({ itemDetails }) {
    ...
    (async () => {
      ...
      await setDetailOptions(
        [
          ...
          store.resellers.length > 0 &&
          purchaseInfo.sellerId === auth.userId
            ? {
                label: "Best Price",
                field: (
                  <div className={s.priceWrapper}>
                    <div
                      className={`flex flex-column justify-content-between ${s.priceInfo}`}
                    >
                      <div className={`${s.priceContainer} flex`}>
                        <span className={`icon-robux ${s.priceIcon}`} />
                        <span
                          className={s.priceLabel}
                          style={{ color: "var(--robux-color)" }}
                        >
                          {formatNum(purchaseInfo?.price || 0)}
                        </span>
                      </div>
                      <span className={`${s.detailTxt} invis767`}>
                    ...
                    <ActionButton
                      ...
                      disabled={true}
                    />
                    <span className={`${s.detailTxt} invis-767`}>
                      You are selling this item.
                    </span>
                  </div>
                ),
            ...
            : (itemDetails.itemRestrictions.includes("Limited") ||
              ...
              !itemDetails.isForSale &&
              store.resellers.length > 0 &&
              (store.resellers.length !== 1 ||
                store.resellers[0].seller.id !== auth.userId)
            ? {
                label: "Best Price",
                field: (
                  <div className={s.priceWrapper}>
                    ...
                    >
                      ...
                      <span className={`${s.detailTxt} invis767`}>
                        ...
                        <a
                          ...
                          onClick={() => {
                            document
                              .getElementById("itemResellerHeader")
            ...
            ? {
                ...
                field: (
                  <div className={s.noPriceContainer}>
                    <span className={`${s.detailTxt} top invis-767`}>
                      No one is selling this item currently.
                    </span>
                    <ActionButton
                      className={s.buyBtn}
                      label="Buy"
                      buttonStyle={buttonStyles.newBuyButton}
                      divClassName={s.buyBtnContainerr}
                      disabled={true}
                    />
                  </div>
                ),
            ...
            : store.isOwned &&
              AssetTypeCategory.All.includes(itemDetails.assetType)
            ? {
                ...
                field: (
                  <div className={s.editWrapper}>
                    <Link
                      href={
                        getAvPageStyle() === avPageStyleType.Legacy
                          ? "/My/Character.aspx"
                          : "/My/Avatar"
                      }
                    >
                  ...
                  <div className={s.priceWrapper}>
                    <div className={`${s.priceContainer} flex`}>
                      {
                        ...
                        itemDetails.priceTickets ? (
                          <>
                            <span
                              className={`icon-tix ${s.priceIcon}`}
                              style={
                                itemDetails.priceTickets === 0
                                  ? { display: "none" }
                                  : {}
                              }
                            />
                            <span
                              className={s.priceLabel}
                              style={{ color: "var(--tix-color)" }}
                            >
                              {(itemDetails.priceTickets === 0 && "Free") ||
                                formatNum(itemDetails.priceTickets)}
                            </span>
                          </>
                        ) : (
                          <>
                            <span
                              className={`icon-robux ${s.priceIcon}`}
                              style={
                                itemDetails.price === 0
                                  ? { display: "none" }
                                  : {}
                            ...
                            <span
                              ...
                              style={{ color: "var(--robux-color)" }}
                            >
                              {(itemDetails.price === 0 && "Free") ||
                                formatNum(itemDetails.price)}
                            </span>
                          </>
                        )
                      }
                    ...
                    <ActionButton
                      ...
                      divChildren={
                        ...
                        new Date() < new Date(itemDetails.offsaleDeadline) ? (
                          <div
                            className={`${s.saleClockContainer}`}
                            // offsaleBy={itemDetails.offsaleDeadline}
                          >
                            Offsale in
                            <Countdown
                              className={s.saleClock}
                              timestamp={itemDetails.offsaleDeadline}
                            />
                          </div>
                        ) : null
                      }
                      label={`${
                        purchaseInfo?.expectedPrice === 0 ? "Get" : "Buy"
                      }`}
                      buttonStyle={`${buttonStyles.newBuyButton} ${
                        itemDetails.priceTickets ? "tix" : ""
                      }`}
              ...
              },
          {
            label: " ",
            field: (
              <pre className={s.itemDescription}>
                {itemDetails.description?.replace(/\\n/g, "\n") ||
              ...
              <div>
                {itemDetails.genres.map((genre, ind) => (
                  <>
                    <Link
                      href={`/catalog?genres=${
                        genresList.find((d) => d.name === genre).genre
                      }`}
                    ...
                    {itemDetails.genres.length - 1 !== ind ? (
                      <span>, </span>
                    ) : null}
                  </>
                ))}
  ...
  const ItemThumb = ({ isVisible }) => (
    <div
      className={`${s.itemThumbContainer} flex flex-column ${
        !isVisible ? "invisible-1" : null
      }`}
    >
      <div
        className={`w-fit-content position-relative ${s.itemThumb} ${
          itemDetails.assetType === AssetType.Badge ||
          itemDetails.assetType === AssetType.GamePass
            ? s.smallImg
            : ""
        }`}
      >
      ...
      <div
        className={`${s.itemInteractionContainer} ${
          isWindow767 ? "display-none" : ""
        }`}
      >
        <div
          id="desktopInteractionContainer"
          className={`flex ${s.favBtnContainer}`}
        >
          <FavouriteButton
            assetId={itemDetails.id}
            initFavCount={itemDetails.favoriteCount}
          />
        </div>
      </div>
    </div>
        ...
        <div className={s.itemDetailsContainer}>
          <div className={`${s.itemHeaderContainer} flex w-100 flex-column`}>
            ...
            <div className={`flex align-items-center ${s.itemHeaderInfo}`}>
              <span style={{ color: "var(--text-color-secondary)" }}>
                By{" "}
                <CreatorLink
                  type={itemDetails.creatorType}
                  id={itemDetails.creatorTargetId}
                  name={itemDetails.creatorName}
                />
              </span>
              {store.isOwned ? (
                <>
                  ...
                  >
                    Item Owned{" "}
                    {store.isResellable() && store.ownedCopies.length > 0
                      ? `(${store.ownedCopies.length})`
                      : ""}
                  </span>
                </>
              ) : null}
          ...
          <div className={`${s.itemDetails} flex w-100 flex-column`}>
            {detailOptions.map((item, ind) => {
              ...
              const isAfterBuy =
                detailOptions[0] !== item &&
                detailOptions[ind - 1].label.includes("Price");
              return (
                <div
                  className={`${isAfterBuy ? "margin-none" : ""} ${
                    s.attrContainer
                  } ${item.parentClass}`}
                >
                  <span
                    className={`${
                      !item?.labelClass ? s.attrLabel : item.labelClass
                    }`}
                  >
                    {item?.label}
                  </span>
            ...
            {detailOptions.length === 0 ? (
              <span
                className="spinner"
                style={{ backgroundSize: "auto 36px" }}
              />
            ) : null}
          </div>
        </div>
    ...
    <Theme2016>
      <FavouriteButtonStore.Provider>
        <div className="container big noPad767">
          <UserAdvertisement
            type={UserAdvertisementType.Banner728x90}
            wrapperClass={s.bannerAdContainer}
            backupWidth={90}
          />
          <div className={s.pageWrapper}>
            <AssetDetailsPage itemDetails={details} />
            <UserAdvertisement
              type={UserAdvertisementType.SkyScraper160x600}
...
function IsISOWithinDays(iso, days) {
  ...
  return now.valueOf() - then.valueOf() < dayNum;
}
