...
function RelatedGame() {
    ...
    (async () => {
      switch (store.details.assetType) {
        case AssetType.GamePass:
          let universeInfo = await getGamePassRootPlace({
          ...
          setGameUrl(
            getGameUrl({ placeId: universeInfo.id, name: universeInfo.name })
          );
          break;
        case AssetType.Badge:
          ...
          setGameUrl(
            getGameUrl({
              ...
              name: badgeInfo.awardingUniverse.name,
            })
          );
          break;
      }
  ...
  if (!store.details || !placeId || placeId === 0) {
    console.log(`dd ${placeId} ff ${placeName}`);
    return null;
  }

  return (
    <div className={s.relatedAssetContainer}>
      <div className={s.assetName}>
        <span>
          {store.details.assetType === AssetType.GamePass
            ? "Use this Game Pass"
            : "Earn this Badge"}{" "}
          in:{" "}
          <Link href={gameUrl}>
