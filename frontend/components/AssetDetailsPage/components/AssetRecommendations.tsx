...
function AssetRecommendationEntry({ recom, assetType }) {
  ...
  return (
    <div
      className={`section-content hoverShadow flex flex-column ${s.recomCardContainer}`}
    >
      <div className="flex flex-column" style={{ paddingBottom: 5 }}>
        <Link href={href} className="flex flex-column">
          <div className={s.thumbContainer}>
            <ItemImage name={recom.item.name} id={recom.item.assetId} />
          </div>
          <span className={`${s.nameContainer} ${s.text}`}>
            {recom.item.name}
          </span>
        </Link>
        <div className={`${s.creatorContainer} ${s.text} flex w-fit-content`}>
          <span
            style={{ marginRight: 3, color: "var(--text-color-secondary)" }}
          >
            By
          </span>
          <CreatorLink
            id={recom.creator.creatorId}
            type={recom.creator.creatorType}
            name={recom.creator.name}
          />
        </div>
        <div className={`${s.text} flex w-fit-content`}>
          {recom.product.priceInRobux ? (
            <>
              ...
              <span className={`${s.currencyText} text-robux`}>
                {abbreviateNumber(recom.product.priceInRobux)}
              </span>
            </>
          ) : recom.product.priceInTickets ? (
            <>
              <span className={`${s.currencyIcon} icon-tix-16x16`} />
              <span className={`${s.currencyText} text-tix`}>
                {abbreviateNumber(recom.product.priceInTickets)}
              </span>
            </>
          ) : (
            <span className={`${s.currencyText} text-robux`}>
              {recom.product.noPriceText || "Free"}
            </span>
          )}
        </div>
      </div>
...
export const getCreatorUrl = (creatorId, creatorType) =>
  creatorType === CreatorType.Group
    ? `/users/${creatorId}/profile`
    : `/My/Groups.aspx?gid=${creatorId}`;
