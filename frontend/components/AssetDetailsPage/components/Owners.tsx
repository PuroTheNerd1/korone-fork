...
function Owners({ isLabelHidden = false }) {
  ...
  useEffect(() => setOwners(store.owners.slice(0, 10)), [store.owners]);
  ...
  if (owners.length === 0)
    return (
      <div>
        ...
        <div
          className={`section-content-off noShadow ${s.resellersWrapper}`}
          style={{ padding: 15 }}
        >
    ...
    <div>
      ...
      <div className={`section-content noShadow ${s.resellersWrapper}`}>
        {owners.map((owner) => {
          ...
          return (
            <div className={s.resellerContainer}>
              <Link href={href} className={s.resellerImg}>
                <img
                  src={ThumbnailFromState(
                    owner.imageUrl || "",
                    owner.state || ""
                  )}
              ...
              <div className={s.resellerInfo}>
                ...
                <span className={s.serialText}>
                  Serial{" "}
                  {owner.serialNumber
                    ? `#${owner.serialNumber} of ${store.resaleData.sales}`
                    : "N/A"}
                </span>
                <div className={`${s.priceContainer} flex`}>
                  <span className={s.priceLabel}>
                    {dayjs(owner.updated).fromNow()}
                  </span>
                </div>
              </div>
              <div className={s.btnContainer2}>
                ...
                (owner.owner.id !== auth.userId &&
                  ...
                  <ActionButton
                    ...
                    onClick={() => {
                      window.open(
                        "/Trade/TradeWindow.aspx?TradePartnerID=" +
                          owner.owner.id,
                        "_blank",
