...
const AddPassEntry = ({ universeId, forceHeight }: {universeId: number; forceHeight?: boolean;}) => {
  const s = useStyles();
  const url = getGamePassCreationUrl({ universeId });
  
  return <div className={s.gPassWrapper} style={forceHeight ? { height: 245 } : null}>
    <Link href={url} className={`section-content hoverShadow ${s.gPassContainer} ${s.addPassContainer}`}>
