...
const downloadProjexModal = (props) => {
  ...
  const onClick = (e) => {
    ...
    window.location.href =
      "https://setup.pekora.zip/ProjectXPlayerLauncher.exe";
  };
  ...
  return (
    <>
      {!props.closeModals && !isOpen && (
        <InstructionsModal
          closeModals={props.closeModals}
          exitFunction={props.exitFunction}
        />
      )}
      {!props.closeModals && isOpen && (
        <NewModal
          ...
          title=""
          footerElements={
            <Link href="/help/install" className={`link2018 ${s.footerLink}`}>
              Click here for help
            </Link>
          }
        >
          ...
          <div className={s.buttons}>
            <ActionButton
              ...
              label="Download and Install Pekora"
              className={s.button2}
            />
          </div>
        </NewModal>
