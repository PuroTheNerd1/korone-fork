...
const useSignalCoreForRealTimeChat = getFlag(
  "useSignalCoreForRealTimeChat",
  false
);
...
const GenerateLayoutFromMessageHistory = (chat) => {
  ...
  chat.forEach((v, i, arr) => {
    ...
    const previousInFinal =
      final.length > 0 ? final[final.length - 1] : undefined;
    ...
    if (previousInFinal && previousInFinal.type === "message") {
      if (previousInFinal.data.senderTargetId === v.senderTargetId) {
        ...
        const lastSendTime = dayjs(
          previousInFinal.data.messages[
            previousInFinal.data.messages.length - 1
          ]
        );
    ...
    if (previousInFinal && previousInFinal.type === "message") {
      ...
      if (currentTime.day() !== lastSendTime.day()) {
        let doAddFull = true;
        if (
          currentTime.month() === lastSendTime.month() &&
          currentTime.year() === lastSendTime.year()
        ) {
      ...
      <>
        {props.messageLayout.map((v) => {
          if (v.type === "message") {
            ...
            return (
              <div
                key={v.data.messages[0].id}
                className={
                  historyStyles.box +
                  " " +
                  (isMe ? historyStyles.boxSelf : historyStyles.boxOther)
                }
              >
                {isMe ? (
                  <>
                    <div className={historyStyles.messageBoxSelf}>
                      {v.data.messages.map((v) => {
                        return (
                          <p key={v.id} className={historyStyles.message}>
                            {v.content}
                          </p>
                        );
                      })}
            ...
            return (
              <div key={"date " + v.data.label} className={historyStyles.date}>
                <p className={historyStyles.dateLabel}>
                  <span className={historyStyles.dateSpan}>{v.data.label}</span>
                </p>
              </div>
            );
          }
...
const Conversation = (props: ConversationProps) => {
  ...
  useEffect(() => {
    const myConvo = store.conversations.filter(
