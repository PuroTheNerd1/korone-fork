...
const CreateGame = (props) => {
  ...
  const [gameName, setGameName] = useState(
    `${auth.username}'s Place Number: 0`
  );
  ...
  useEffect(() => {
    (async () => {
      setGameName(
        `${auth.username}'s Place Number: ${
          (await getUserCreatedPlaceCount({ userId: auth.userId })) + 1
        }`
      );
    })();
  }, []);
  ...
  const options = [
    ...
    },
    {
      ...
      element: (
        <AdvancedSettings
          uncopylocked={uncopylocked}
          setUncopylocked={setUncopylocked}
        />
      ),
    },
  ];
  ...
  const createGame = async () => {
    ...
    if (gameName.length < 3 || gameName.length > 100) {
      feedback.addFeedback(
        "Game name is too long or too short. You figure it out, loser.",
        FeedbackType.ERROR
      );
      setLocked(false);
      return;
    ...
    if (gameDescription && gameDescription.length > 1000) {
      feedback.addFeedback(
        "Game description is too long or too short.",
        FeedbackType.ERROR
      );
      setLocked(false);
      return;
    ...
    let placeId = await createGameRequest({
      templatePlaceId: selectedTemplate,
    });
    ...
    if (!placeId) {
      feedback.addFeedback(
        "Failed to create game due to an unexpected error.",
        FeedbackType.ERROR
      );
      setLocked(false);
      return;
    ...
    await setUniverseMaxPlayers({
      universeId: placeId.universeId,
      maxPlayers: playerCount,
      verbose: true,
    });
    await setUniverseYear({
      universeId: placeId.universeId,
      year: gameYear,
      verbose: true,
    });
  ...
  return (
    <div className={`container ssp ${s.WillTheRealSlimShadyPleaseStandUp}`}>
      {locked && (
      ...
      <div>
        <OldVerticalTabs
          contentStyles={`${s.contentContainer} vTabContent`}
          options={options}
          default={tab}
      ...
      <div className={s.actionContainer}>
        <ActionButton
          className={s.properPadding}
          buttonStyle={but.buyButton}
          label="Create Game"
          onClick={async (e) => {
            e.preventDefault();
            await createGame();
