...
public class PlaceLauncherService : ServiceBase
{
    ...
    public async Task<PlaceLaunchResponse> RequestGame(long placeId, int matchmaking, string cookie, bool? Special = false, string? username = null, long? userId = null)
    {
        ...
        if ((bool)Special)
        {
            ...
            joinScript = await games.GetJoinScript(year, username, (long)userId, result.job, placeId, uni.universeId, uni.builderId, characterAppearanceUrl, finalTicket, membership, accountAgeDays, true, cookie, null);
        }
