...
public class FrontendProxyMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext ctx)
    {
        ...
        if (requestUrl.Contains("/canmanage/") || requestUrl.Contains("filter-friends")|| requestUrl.Contains("AbuseReport") || Regex.IsMatch(requestUrl, @"^/places/\d+/settings$") || Regex.IsMatch(requestUrl, @"^/users/\d+$") || Regex.IsMatch(requestUrl, @"^/Users/\d+$") || requestUrl.Contains("/universes/"))
        {
            await _next(ctx);
            return;
        }
