...
public class FrontendProxyMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext ctx)
    {
        ...
        if (requestUrl.Contains("/canmanage/") || Regex.IsMatch(requestUrl, @"^/users/\d+$") || Regex.IsMatch(requestUrl, @"^/Users/\d+$") || Regex.IsMatch(requestUrl, @"^/places/\d+/settings$", RegexOptions.IgnoreCase) || requestUrl.Contains("/universes/"))
        {
            await _next(ctx);
            return;
        }
